//family.js

var family= {
  'bbmelbhnoccdkjiblgchdaofjdbombmh': {
    'name':'Chromapaper Core',
    'short':'Core',
    'description':'Features a Browser Button and Context Menu integration.',
    'url':'https://chrome.google.com/webstore/detail/bbmelbhnoccdkjiblgchdaofjdbombmh',
    'options_page':'options.html'
  },
  //'lpjpjcgbkjefppoahpegfajifjdmcblb' - Stable
  //Replace with the ID of your local unpacked dir if you're developing.
  'afgnnmjingbmbpcfhennhnnklngeadel': {
    'name':'Chromapaper Reader',
    'short':'Reader',
    'description':'A Chrome app that includes offline reading, a paginated view, and scroll tracking to save your place.',
    'url':'https://chrome.google.com/webstore/detail/lpjpjcgbkjefppoahpegfajifjdmcblb',
    'options_page':'options.html'
  },
  'anphalfoaaegidcbognealancgeanllm': {
    'name':'Chromapaper Keyboard Shortcuts',
    'short':'Shortcuts',
    'description':'Adds customizable keyboard shortcuts for various tasks.',
    'url':'https://chrome.google.com/webstore/detail/anphalfoaaegidcbognealancgeanllm',
    'options_page':'options.html'
  }
};
